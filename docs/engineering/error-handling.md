...
诊断证据包在读取本次来源 Artifact 时发现大小或 SHA-256 不一致，返回
`DIAGNOSTIC_SOURCE_INVALID`；来源或最终 ZIP 超过 24 MiB 返回 `DIAGNOSTIC_BUNDLE_TOO_LARGE`；
最终 ZIP 文件集合未通过固定清单验证返回 `DIAGNOSTIC_BUNDLE_NOT_VERIFIED`。三者均为已知失败，
不得发布未验证的 Bundle；任务报告可以保留已经完成的来源 Artifact 引用，但不得内联内容。

本地清理确认后出现路径、大小、时间或 SHA-256 变化，返回
`LOCAL_DATA_CLEANUP_SCOPE_CHANGED`；已批准文件无法删除返回 `LOCAL_DATA_CLEANUP_FAILED`；
删除后仍存在返回 `LOCAL_DATA_CLEANUP_NOT_VERIFIED`。清理不自动重试，失败报告保留已完成删除的
Artifact ID 与字节摘要，不包含文件内容或任意路径。
